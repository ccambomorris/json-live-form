<template v-if="mounted">
  <div class="sf-nested-rows">
    <table>
      ...
      <tr
        v-for="(row, rowIndex) in modelValue[config.field!]"
        :key="`row-${rowIndex}`"
        class="sf-nested-row"
      >
        <td
    ...
    <button
      class="sf-nested-row add-row"
      tabindex="0"
      @click="insertNestedItem"
    >
      Add Entry
    </button>
  </div>
...
<script setup lang="ts">
  ...
  let insertNestedItem = () => {
    ...
    for (const index in columnSchema) {
      ...
      newObject[columnSchema[index]['field']] =
        columnSchema[index]['type'] === 'nested' ? [] : null;
    }
